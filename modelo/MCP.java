...
public class MCP implements TreinamentoPadrao {
	
	private double taxaDeAprendizado = 0.05;

	private double erroMinimo = 0.01;

	private int numeroMaximoCiclos = 50000;

	private ArrayList<Padrao> padroes = new ArrayList<Padrao>();
	
	private Camada camadaEntrada = new CamadaEntrada ("C0", 25, 1);
	
	private Camada camadaIntermediaria = new CamadaIntermediaria ("C1", 12, this.camadaEntrada.getNumeroNeuronios());
	...
	public void adicionarPadroesDefault() {
		int e = 0;
		for (Representacao r : MCP.REPRESENTACOES) {
			this.adicionarPadrao(MCP.ELEMENTOS[e], r, MCP.SAIDAS[e]);
			e++;
		}
	}
